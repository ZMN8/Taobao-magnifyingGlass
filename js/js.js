...
for(i=0;i<liNodes.length;i++){
    ...
    liNodes[i].onmouseenter= function () {
        ...
        for(j=0;j<liNodes.length;j++){
            if(liNodes[j].className.indexOf("main_imgCur")!=-1){
                oldPos=j;
                break;
            }
        }
        liNodes[oldPos].className="";//去除旧位置的样式
        this.className="main_imgCur";//当前位置加上样式
        var smallImgSrc=this.getElementsByTagName("img")[0].src;
        MiddleImg.src=smallImgSrc.substring(0,smallImgSrc.length-5)+"m.jpg";
        BigImg.src=smallImgSrc.substring(0,smallImgSrc.length-5)+"b.jpg"
    }
}

middleDiv.onmousemove= function (e) {
    var moveIcon=this.getElementsByTagName("span")[0];//放大镜
    var event=window.event||e;
    var mouseX=event.pageX;//鼠标相对于网页左边的水平距离
    var mouseY=event.pageY;//鼠标相对于网页上边的垂直距离
    var divPosX=this.offsetLeft+this.parentNode.offsetLeft;//middleDiv相对于body左边的距离
    var divPosY=this.offsetTop+this.parentNode.offsetTop;//middleDiv相对于body上边的距离
...
middleDiv.onmouseleave= function () {
    var moveIcon=this.getElementsByTagName("span")[0];
